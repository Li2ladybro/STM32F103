...
const chinese_cell_t OlED_CF16x16[]=
{
    // 定义一个汉字字符库，只添加自己需要的
    ...
    0x08,0x08,0x08,0x08,0x09,0x49,0x89,0x79,0x0D,0x0B,0x09,0x08,0x08,0x08,0x08,0x00,/*2*/
    "梦",
    0x40,0x44,0x24,0x14,0x7F,0x94,0x24,0x00,0x24,0x14,0x7F,0x14,0x24,0x44,0x40,0x00,
    0x00,0x80,0x88,0x84,0x42,0x45,0x29,0x11,0x11,0x09,0x05,0x03,0x00,0x00,0x00,0x00,/*3*/
    "非",
    0x08,0x88,0x88,0x88,0x88,0xFF,0x00,0x00,0x00,0xFF,0x88,0x88,0x88,0x88,0x08,0x00,
    0x10,0x10,0x10,0x10,0x10,0xFF,0x00,0x00,0x00,0xFF,0x10,0x10,0x10,0x10,0x10,0x00,/*4*/
    
    // ...
    "",
    0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,
    0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,0xff,/* 结束标志位 */
    
};

uint8_t OLED_DisplayBuf[8][128];  // page * x =1kb
...
void OLED_WriteCommand(uint8_t Comand)
{
    ...
    MyI2C_SendByte(Comand);           // Data Byte
    MyI2C_ReceiveAck();
    MyI2C_Stop();
}
...
{
    OLED_ClearArea(x, y, width, hight);
    for (uint8_t page = 0; page < (hight - 1) / 8 + 1; page++)
    {
        // 分页显示
        for (uint8_t i = 0; i < width; i++)
       {
           // 一次写一列，一个字节
           OLED_DisplayBuf[y / 8 + page][x+i] |= image[page * width + i] << (y % 8);
           OLED_DisplayBuf[y / 8 + page + 1][x+i] |= image[page * width + i] >> (8 - (y % 8));
       } 
    }
//    OLED_DisplayBuf[0][3] = image[1] << 3;
//    OLED_DisplayBuf[1][3] = image[1] >> 5;    
//    
//    OLED_DisplayBuf[0][4] = image[2] << 3;
//    OLED_DisplayBuf[1][4] = image[2] >> 5;    
//    
//    OLED_DisplayBuf[0][5] = image[3] << 3;
//    OLED_DisplayBuf[1][5] = image[3] >> 5;    
//    
//    OLED_DisplayBuf[0][6] = image[4] << 3;
//    OLED_DisplayBuf[1][6] = image[4] >> 5;
}
...
//          OLED_DisplayBuf[Page][x+i] = OLED_F6x8[ch-' '][i];
//        }
    }
...
void OLED_ShowString(uint8_t x,uint8_t y,char* str,uint8_t FrontSize)
{
    ...
    {
        OLED_ShowChar(x+i*FrontSize,y,str[i],FrontSize);
    }
}
...
void OLED_ShowChinese(uint8_t x,uint8_t y,const char* chinese)
{
    ...
    for (uint8_t i = 0; chinese[i] != '\0'; )
    {
        ...
        if (pchinese >= 2)
        {
            ...
            {
                if (strcmp(OlED_CF16x16[pindex].index,single_chinese) == 0)
                {
                    // 找到需要显示汉字在字符库中的索引
                    break;
                }
            ...
            // 计算偏移量 2，4，6，8
            uint8_t gap = ((i / 2) - 1) * 16;
            OLED_ShowImage(x + gap, y, 16, 16,OlED_CF16x16[pindex].data);
        }
    }
...
{
    if (OLED_DisplayBuf[y / 8][x] &= 0x01 << (y%8) == 1)
    {
        return 1;
    }
    return 0;
...
void OLED_DrawLine(uint8_t x1, uint8_t y1, uint8_t x2, uint8_t y2)
{
    ...
    if (fabs(k) < 1)
    {
        ...
        {
            OLED_DrawPoint(x ,round(y1 + (x - x1) * k));
        }
    }
    ...
    else
    {
        ...
        {
            OLED_DrawPoint(round(x1 + (y - y1) / k), y);
        }
    }
}
