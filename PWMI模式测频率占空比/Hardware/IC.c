...
void IC_Init()
{
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_6;
 	GPIO_Init(GPIOA, &GPIO_InitStructure); 					   // 初始化引脚配置
	...
	//TIM_ICInitStructure.TIM_ICFilter=0xF;  							// 滤波
	//TIM_ICInitStructure.TIM_ICPolarity=TIM_ICPolarity_Falling;  		// 边沿检测模式
	//TIM_ICInitStructure.TIM_ICPrescaler=TIM_ICPSC_DIV1;    			// 咩次触发生效
	//TIM_ICInitStructure.TIM_ICSelection=TIM_ICSelection_IndirectTI;	// 直连通道
	// TIM_ICInit(TIM3,&TIM_ICInitStructure);
	
	TIM_PWMIConfig(TIM3,&TIM_ICInitStructure);						    // 与通道一配置反向
